...
function runAsarCommand(action, args) {
    ...
    if (!asarCli) {
        console.error('[エラー] ローカルの @electron/asar CLI が見つかりません。');
        console.error('  先にプロジェクトディレクトリで npm install を実行してから、再度インストールを実行してください。');
        return { success: false, stdout: '', stderr: 'ローカルの @electron/asar が未インストール' };
    }
...
function checkEnvironment() {
    ...
    if (!asarCli) {
        console.error('');
        console.error('╔══════════════════════════════════════════════════════════╗');
        console.error('║  [環境チェック] ローカルの @electron/asar CLI が見つかりません ║');
        console.error('║                                                          ║');
        console.error('║  先にプロジェクトルートディレクトリで実行してください：           ║');
        console.error('║    npm install                                            ║');
        console.error('║                                                          ║');
        console.error('║  完了後に再度インストールスクリプトを実行してください。             ║');
        console.error('╚══════════════════════════════════════════════════════════╝');
        console.error('');
        return false;
    }
    console.log(`[環境チェック] ローカルの asar CLI の準備が完了しました：${asarCli}`);
    return true;
}
...
function loadDictionary() {
    ...
    if (!fs.existsSync(dictsDir)) {
        console.warn(`[警告] 辞書ディレクトリが見つかりません：${dictsDir}`);
        return totalMap;
    }
    ...
    for (const file of files) {
        try {
            ...
            console.warn(`[警告] 辞書ファイルを読み込めません：${file}`);
...
function closeAntigravityProcesses() {
    console.log('[1] ファイルのロックを避けるため、Antigravity を終了しています...');
...
function detectInstallationDir(manualDir) {
    if (manualDir) {
        ...
        console.error(`[エラー] 指定されたインストールパスが存在しません：${manualDir}`);
        process.exit(1);
    }
    ...
    for (const candidate of candidates) {
        if (fs.existsSync(candidate)) {
            console.log(`[検出] Antigravity のインストールディレクトリを見つけました：${candidate}`);
            return path.resolve(candidate);
        }
    }

    console.error('[エラー] Antigravity のインストールディレクトリが見つかりません。--install-dir でパスを指定してください。');
    process.exit(1);
}
...
function createTrayCreatePatch() {
    ...
    const translations = {
        ...
        'Open Antigravity': 'Antigravity を開く',
        'Quit': '終了'
    };
...
function install20(resourcesDir) {
    ...
    if (!fs.existsSync(asarPath)) {
        console.error(`[エラー] resources ディレクトリに app.asar が見つかりません：${resourcesDir}`);
        return false;
    }
    ...
    if (!fs.existsSync(bakPath)) {
        console.log('[バックアップ] 公式の app.asar のバックアップを作成しています...');
        let backupOk = false;
        try {
            ...
            if ((copyErr.code === 'EPERM' || copyErr.code === 'EACCES') && process.platform !== 'win32') {
                console.warn(`[バックアップ] fs.copyFileSync に失敗しました (${copyErr.code})。/bin/cp -p でバックアップの作成を試みます...`);
                const cpResult = runCommandSync(`/bin/cp -p "${asarPath}" "${bakPath}"`);
                if (cpResult.success && fs.existsSync(bakPath)) {
                    backupOk = true;
                    console.log('[バックアップ] /bin/cp -p によるバックアップの作成に成功しました。');
                } else {
                    console.error('[エラー] バックアップ失敗：app.asar.bak を作成できません。');
                    console.error('  考えられる原因：macOS の権限制限、または .app ディレクトリの権限不足。');
                    console.error('  提案：');
                    console.error('    1. 権限のあるターミナルでこのスクリプトを実行してみてください');
                    console.error('    2. または手動でバックアップを作成します：');
                    console.error(`       cp "${asarPath}" "${bakPath}"`);
                    return false;
                }
            } else {
                console.error(`[エラー] バックアップ失敗：${copyErr.message}`);
                return false;
            }
        }
        if (backupOk) {
            console.log('[バックアップ] バックアップが完了しました。');
        }
    } else {
        console.log('[バックアップ] 既に app.asar.bak が存在します。既存のバックアップを使用します。');
    }
    ...
    console.log('[展開] app.asar を展開しています...');
    const extractRes = runAsarCommand('extract', [asarPath, tempDir]);
    if (!extractRes.success || !fs.existsSync(tempDir)) {
        console.error('[エラー] 展開に失敗しました。npm install を実行したか、Node.js が正常に使用できるか確認してください。');
        console.error(`詳細：${extractRes.stderr}\n${extractRes.stdout}`);
        return false;
    }

    const preloadPath = path.join(tempDir, 'dist', 'preload.js');
    if (!fs.existsSync(preloadPath)) {
        console.error(`[エラー] 展開後に preload.js が見つかりません：${preloadPath}`);
        fs.rmSync(tempDir, { recursive: true, force: true });
        return false;
    }

    console.log('[変更] 日本語辞書を注入しています...');
    ...
    console.log('[変更] preload.js への注入が完了しました。');

    const menuPath = path.join(tempDir, 'dist', 'menu.js');
    if (fs.existsSync(menuPath)) {
        console.log('[変更] システムメニューのテキストを注入しています...');
        ...
        if (idx !== -1) {
            ...
            console.log('[変更] システムメニューのテキストの注入が完了しました。');
        } else {
            console.warn('[警告] menu.js の挿入ポイントが見つかりません。システムメニューのテキスト注入をスキップします。');
        }
    }

    const trayPath = path.join(tempDir, 'dist', 'tray.js');
    if (fs.existsSync(trayPath)) {
        console.log('[変更] トレイメニューのテキストを注入しています...');
...
function restore20(resourcesDir) {
    ...
    if (!fs.existsSync(bakPath)) {
        console.log('[ヒント] app.asar.bak が見つかりません。このツールがまだ適用されていないか、バックアップが削除された可能性があります。');
        return false;
    }

    closeAntigravityProcesses();

    console.log('[復元] 公式の app.asar を復元しています...');
    fs.copyFileSync(bakPath, asarPath);
    fs.unlinkSync(bakPath);
    console.log('[完了] 公式の app.asar が復元されました。');
    return true;
}
...
function main() {
    ...
    if (!fs.existsSync(resourcesDir)) {
        console.error(`[エラー] 有効な resources ディレクトリを特定できません：${resourcesDir}`);
        process.exit(1);
    }
    ...
    if (!fs.existsSync(asarPath) && !(restore && fs.existsSync(bakPath))) {
        console.error('[エラー] app.asar が見つかりません。このツールは Antigravity 2.0 のみをサポートしています。');
        process.exit(1);
    }

    if (restore) {
        console.log(`====== ${PROJECT_NAME} を復元しています ======`);
        restore20(resourcesDir);
    } else {
        console.log(`====== ${PROJECT_NAME} を適用しています ======`);
