...
Antigravity 2.0 のインターフェースを日本語に翻訳します。Windows と macOS をサポートし、ワンクリックインストールと完全復元を提供します。
...
- 🔧 **ワンクリックインストール**：スクリプトをダブルクリックするだけでインストール完了
- 🔄 **完全復元**：いつでも公式英語版に復元可能
...
1. `antigravity2-ja-jp-v1.0.0.zip` をダウンロード（バージョン番号は最新のものを選択してください）
2. 任意のディレクトリに解凍
3. 解凍したディレクトリで以下を実行：
